...
{
  date
  echo "Starting MEMIT official full run with cached 100k covariance stats"
  echo "Using persistent HF datasets cache: ${HF_DATASETS_CACHE}"
  echo "Using hparams: /root/autodl-tmp/EasyEdit/hparams/MEMIT/llama3-8b-official-safe.yaml"
  exec /root/miniconda3/envs/EasyEdit/bin/python scripts/run_sequential_edit.py \
    ...
    --method MEMIT \
    --hparams /root/autodl-tmp/EasyEdit/hparams/MEMIT/llama3-8b-official-safe.yaml \
...
} > /root/autodl-tmp/logs/memit_knowedit_zsre_official_full.log 2>&1
