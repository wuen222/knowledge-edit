# A800 实验运行指南

这份文档按“从零开 AutoDL 实例到拿到图表”的顺序写。真实模型编辑和 hidden-state 抽取都在 AutoDL A800 上跑，本地只负责代码管理、结果下载和论文整理。

## 1. 开实例前确认

- GPU 选 A800 80GB。
- 镜像选 PyTorch + CUDA + Python 3.10。
- 数据盘至少 200GB，推荐 300GB。
- Hugging Face 账号已经申请并通过 `meta-llama/Meta-Llama-3-8B-Instruct` 权限。
- 本项目已经能通过 Git clone，或者准备好从 Windows 用 `scp` 上传。

## 2. 初始化目录和缓存

```bash
source /root/.bashrc
nvidia-smi
mkdir -p /root/autodl-tmp/{models,data,runs,artifacts,cache,logs}
echo 'export HF_HOME=/root/autodl-tmp/cache' >> ~/.bashrc
echo 'export TRANSFORMERS_CACHE=/root/autodl-tmp/cache/transformers' >> ~/.bashrc
source ~/.bashrc
```

这样做的目的是把模型缓存和实验结果放到数据盘，避免写满系统盘。

## 3. 安装 EasyEdit

```bash
cd /root
git clone https://github.com/zjunlp/EasyEdit.git
cd /root/EasyEdit
pip install -r requirements.txt
pip install -e .
pip install "transformers>=4.40.0"
```

安装后可以检查 hparams：

```bash
find /root/EasyEdit/hparams -maxdepth 2 -type f | head
find /root/EasyEdit/hparams -maxdepth 2 -iname "*llama*"
find /root/EasyEdit/hparams -maxdepth 2 -iname "*alpha*"
```

如果某个方法找不到 Llama 配置，后面运行时需要显式传入 `--hparams`。

## 4. 部署本项目

有 Git 仓库时：

```bash
cd /root
git clone <你的仓库地址> knowledge-editing-drift
cd /root/knowledge-editing-drift
pip install -r requirements.txt
pip install -e .
```

没有 Git 仓库时，在本地 PowerShell 运行：

```powershell
scp -rP <端口> "E:\New project" root@<AutoDL地址>:/root/knowledge-editing-drift
```

然后回到 AutoDL：

```bash
cd /root/knowledge-editing-drift
pip install -r requirements.txt
pip install -e .
```

## 5. 登录 Hugging Face

```bash
huggingface-cli login
python - <<'PY'
from transformers import AutoTokenizer
AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3-8B-Instruct")
print("Llama 3 权限和缓存路径正常")
PY
```

如果这里失败，优先处理权限问题。可以临时用 `Qwen/Qwen2.5-7B-Instruct` 调试工程流程，但主实验不要混用模型。

## 6. 准备 ZsRE 数据

把 `zsre_mend_eval.json` 放到：

```bash
/root/autodl-tmp/data/zsre_mend_eval.json
```

然后运行：

```bash
cd /root/knowledge-editing-drift
python scripts/prepare_data.py --dataset zsre --n-edits 100 --seed 42
```

正常情况下会生成：

```bash
/root/autodl-tmp/data/prepared/zsre/edits.jsonl
/root/autodl-tmp/data/prepared/zsre/probes.jsonl
/root/autodl-tmp/data/prepared/zsre/manifest.json
```

## 7. 先跑冒烟测试

```bash
python scripts/run_sequential_edit.py \
  --model meta-llama/Meta-Llama-3-8B-Instruct \
  --method ROME \
  --checkpoints 0 1 2 \
  --max-edits 2 \
  --probe-limit 20 \
  --batch-size 1 \
  --hidden-batch-size 1 \
  --easyedit-root /root/EasyEdit \
  2>&1 | tee /root/autodl-tmp/logs/smoke_rome.log
```

通过冒烟测试的标志是：

- 日志最后出现“运行完成”。
- `/root/autodl-tmp/runs` 下出现本次运行目录。
- 检查点目录中存在 `metrics.json`、`hidden_drift.csv`、`weight_delta.csv`。

## 8. 正式跑主实验

建议开启 `tmux`：

```bash
tmux new -s ke
cd /root/knowledge-editing-drift
```

按方法逐个跑。你已经完成 `FT-L/ROME/MEMIT/PMET` 时，下一步建议补跑 `AlphaEdit`。如果从零开始，优先顺序建议是 `FT-L -> ROME -> MEMIT -> PMET -> AlphaEdit`。FT-L 最像朴素微调基线，也最容易暴露显存和配置问题；AlphaEdit 用来检验 null-space 扰动约束是否能缓解连续编辑退化。

```bash
python scripts/run_sequential_edit.py --model meta-llama/Meta-Llama-3-8B-Instruct --method FT-L --checkpoints 0 1 10 50 100 --easyedit-root /root/EasyEdit 2>&1 | tee /root/autodl-tmp/logs/ftl.log
python scripts/run_sequential_edit.py --model meta-llama/Meta-Llama-3-8B-Instruct --method ROME --checkpoints 0 1 10 50 100 --easyedit-root /root/EasyEdit 2>&1 | tee /root/autodl-tmp/logs/rome.log
python scripts/run_sequential_edit.py --model meta-llama/Meta-Llama-3-8B-Instruct --method MEMIT --checkpoints 0 1 10 50 100 --easyedit-root /root/EasyEdit 2>&1 | tee /root/autodl-tmp/logs/memit.log
python scripts/run_sequential_edit.py --model meta-llama/Meta-Llama-3-8B-Instruct --method PMET --checkpoints 0 1 10 50 100 --easyedit-root /root/EasyEdit 2>&1 | tee /root/autodl-tmp/logs/pmet.log
python scripts/run_sequential_edit.py --model meta-llama/Meta-Llama-3-8B-Instruct --method AlphaEdit --checkpoints 0 1 10 50 100 --easyedit-root /root/EasyEdit 2>&1 | tee /root/autodl-tmp/logs/alphaedit.log
```

`SCR-LITE` 只建议作为 appendix 或工程 sanity check，不建议放进主表：

```bash
python scripts/run_sequential_edit.py --model meta-llama/Meta-Llama-3-8B-Instruct --method SCR-LITE --checkpoints 0 1 10 50 100 2>&1 | tee /root/autodl-tmp/logs/scr_lite.log
```

每个方法先确认 1 个跑完，再启动下一个方法。这样更容易定位配置错误。

## 9. 生成图表

```bash
python scripts/make_figures.py --runs /root/autodl-tmp/runs --out /root/autodl-tmp/artifacts
```

核心输出包括：

- 行为指标曲线：Reliability、Generalization、Locality、Portability、Average。
- 内部漂移曲线：hidden-state drift、参数 relative L2。
- 参数扰动热力图。
- 层级 hidden-state drift 图。
- PCA/t-SNE/UMAP 投影图。
- token trajectory 图。
- `correlations.csv`：行为退化与内部扰动的 Spearman 相关。

## 10. 监控与排错

监控显存：

```bash
watch -n 5 nvidia-smi
```

查看日志：

```bash
tail -f /root/autodl-tmp/logs/rome.log
```

查看空间：

```bash
du -sh /root/autodl-tmp/*
```

常见问题：

- hparams 找不到：用 `find /root/EasyEdit/hparams -maxdepth 2 -iname "*llama*"` 或 `find /root/EasyEdit/hparams -maxdepth 2 -iname "*alpha*"` 查文件，再加 `--hparams`。
- 显存不足：降低 `--batch-size`、`--hidden-batch-size`、`--probe-limit`。
- Llama 下载失败：重新确认 Hugging Face 权限和 token。
- 图表没有生成：先看 `/root/autodl-tmp/runs` 是否存在各 checkpoint 的 `metrics.json` 和 `hidden_states.npz`。

## 11. 下载结果

Windows 本地：

```powershell
scp -rP <端口> root@<AutoDL地址>:/root/autodl-tmp/artifacts "E:\New project\artifacts"
scp -rP <端口> root@<AutoDL地址>:/root/autodl-tmp/runs "E:\New project\runs"
```

如果目录太大，只下载轻量结果：

```bash
rsync -av --include='*/' --include='*.json' --include='*.jsonl' --include='*.csv' --include='*.png' --include='*.pdf' --exclude='*' \
  root@<AutoDL地址>:/root/autodl-tmp/runs/ ./runs/
rsync -av root@<AutoDL地址>:/root/autodl-tmp/artifacts/ ./artifacts/
```
